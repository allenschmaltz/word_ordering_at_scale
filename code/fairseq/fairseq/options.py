...
def get_eval_lm_ordering_parser(default_task='language_modeling'):
    parser = get_parser('Evaluate Language Model Ordering', default_task)
    add_dataset_args(parser, gen=True)
    add_eval_lm_ordering_args(parser)
    return parser
...
def add_eval_lm_ordering_args(parser):  # added for word ordering
    group = parser.add_argument_group('LM Ordering')
    add_common_eval_args(group)
    # fmt: off
    group.add_argument('--output-word-probs', action='store_true',
                       help='if set, outputs words and their predicted log probabilities to standard output')
    group.add_argument('--output-word-stats', action='store_true',
                       help='if set, outputs word statistics such as word count, average probability, etc')
    group.add_argument('--input_ordered_file', default="",
                       help='The reference input file (already in correct order).')
    group.add_argument('--input_shuffled_file', default="",
                       help='The reference input file (shuffled).')
    group.add_argument('--input_unigram_file', default="",
                       help='Unigram file for future costs (optional).')
    group.add_argument('--output_reordered_filepath_with_name', default="",
                       help='File in which to save the reordered output')
    group.add_argument('--ordering_beam_size', default=10, type=int,
                       help='Beam size for ordering')
    group.add_argument('--ordering_max_batch_size', default=1, type=int,
                       help='Max batch size for ordering if > 0; Else hard-coded batch size schedules. -1 is for a 12gb card.')
    group.add_argument('--ordering_start_sentence', default=0, type=int,
                       help='Starting index of the sentence (in input_shuffled_file) to reorder (indexed from 0)')
    group.add_argument('--ordering_end_sentence', default=0, type=int,
                       help='Final index of the sentence (in input_shuffled_file) to reorder (indexed from 0)')
    # fmt: on
